...
export const usePlayers = () => {
    return useQuery({
        queryKey: ['players'],
        queryFn: async () => {
            const { data } = await api.get<any[]>('/players'); // Use any[] initially to process
            return data.map((p) => {
...
export const usePlayer = (playerId: number | string | undefined) => {
    return useQuery({
        ...
        queryFn: async () => {
            ...
            const { data } = await api.get<any>(`/playersById/${playerId}`);
...
export const useTeam = (teamId: number | string | undefined) => {
    return useQuery({
        ...
        queryFn: async () => {
            ...
            const { data } = await api.get<any[]>(`/teamsById/${teamId}`);
