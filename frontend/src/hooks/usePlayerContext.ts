...
export const usePlayerContext = (player: Player | null | undefined, allPlayers: Player[] | undefined): PlayerContext | null => {
    return useMemo(() => {
        ...
        const calcPer90 = (p: Player, key: string) => {
            ...
            let val = 0;
            if (key in p.stats) val = (p.stats as any)[key] || 0;
            else if (p.fpl_stats && key in p.fpl_stats) val = (p.fpl_stats as any)[key] || 0;
            else if (key in p) val = (p as any)[key] || 0;
            else {
                // Mapping for tricky legacy names if needed
                if (key === 'expectedGoals') val = p.stats.expectedGoals || Number(p.fpl_stats?.expected_goals) || 0;
            }
